...
#     return
def update_rocchio_dict(query, title, title_dict,relevantBool):

    if relevantBool:
        if query in relevant:
            relevant[query] += [title_dict[title]]
        else:
            relevant[query] = [title_dict[title]]
    else:
        if query in irrelevant:
            irrelevant[query] += [title_dict[title]]
        else:
            irrelevant[query] = [title_dict[title]]
    print("relevant: ", relevant)
    print("irrelevant: ", irrelevant)
...
def rocchio(query,num_articles,a=.6, b=.3, clip = True):
    print(relevant)
    print(irrelevant)
    print("query: ", query)
    print("\n")
    ...
    dr = np.zeros(num_articles)
    dnr = np.zeros(num_articles)

    if query in relevant:
        for t in (relevant[query]):
            print("t", t)
            dr[t] +=1
        for t in (irrelevant[query]):
            dnr[t] +=1
    new_weights = a*dr - b*dnr
    if clip:
